...
const app = {
    ...
    showReminder(schedule) {
        ...
        if (schedule) {
            ...
            // Update button text
            if (confirmButton) {
                confirmButton.textContent = isMedication 
                    ? '✓ I Took My Medicine'
                    : '✓ Done';
            }
        }
    ...
    confirmMedication() {
        if (window.currentReminder) {
            MedicationManager.confirmMedication(window.currentReminder.id);
    ...
    updateNextReminder() {
        ...
        if (nextReminderTimeEl) {
            if (nextReminder) {
                ...
                nextReminderTimeEl.textContent = `${nextReminder.schedule.name} at ${time} (in ${timeUntil})`;
            } else {
    ...
    /**
     * Toggle reminder info visibility
     */
    toggleReminderInfo() {
